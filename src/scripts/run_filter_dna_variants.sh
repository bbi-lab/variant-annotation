...
if [[ $# -lt 2 ]]; then
  cat <<'EOF'
Usage: src/scripts/run_filter_dna_variants.sh <input-file> <output-file> [filter_dna_variants options...]
...
Examples:
  ...
  src/scripts/run_filter_dna_variants.sh annotated.tsv filtered.tsv --keep-class snv --keep-class wt
...
cmd+=(--rm --no-deps --entrypoint python map-variants
  -m src.filter_dna_variants "$input_in_container" "$output_in_container")
