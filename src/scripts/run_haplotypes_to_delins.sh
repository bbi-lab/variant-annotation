...
if [[ $# -lt 2 ]]; then
  cat <<'EOF'
Usage: src/scripts/run_haplotypes_to_delins.sh <input-file> <output-file> [options...]

Examples:
  src/scripts/run_haplotypes_to_delins.sh in.tsv out.tsv
  src/scripts/run_haplotypes_to_delins.sh in.tsv out.tsv --raw-hgvs-nt-col raw_hgvs_nt
...
exec docker compose --profile tools run \
  ...
  -m src.normalize_intra_codon_haplotypes "$input_in_container" "$output_in_container" "$@"
