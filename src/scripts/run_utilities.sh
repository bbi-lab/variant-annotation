...
Commands:
  ...
  merge-rows      <output-file> <input-file>... [merge options]
...
Examples:
  ...
  src/scripts/run_utilities.sh merge-rows merged.tsv base.tsv patch.tsv --key-col id
...
if [[ $# -lt 1 ]]; then
  echo "error: missing command (expected: filter-columns, filter-rows, or merge-rows)" >&2
  exit 2
fi
...
case "$command_name" in
  ...
  merge-rows)
    if [[ $# -lt 2 ]]; then
      echo "error: merge-rows requires <output-file> <input-file>..." >&2
      exit 2
    fi
    ...
    if [[ ${#input_paths[@]} -lt 1 ]]; then
      echo "error: merge-rows requires at least one input file" >&2
      exit 2
    fi
    ...
    first_input="${mapped_inputs[0]}"compare-columns, 
    if [[ "$output_path" == /work/* || "$output_path" == /usr/src/app/* ]]; then
      output_in_container="$output_path"
  ...
  *)
    echo "error: unknown command '$command_name' (expected: filter-columns, filter-rows, or merge-rows)" >&2
    exit 2
    ;;
esac
