...
if [[ $# -lt 2 ]]; then
  cat <<'EOF'
Usage: src/scripts/run_reverse_translate_protein_variants.sh <input-file> <output-file> [reverse-translate options...]

Examples:
  src/scripts/run_reverse_translate_protein_variants.sh input.tsv output.tsv
  src/scripts/run_reverse_translate_protein_variants.sh input.tsv output.tsv --include-indels
...
exec docker compose -f docker-compose-dev.yml --profile tools run \
  ...
  reverse-translate-proteins "$input_in_container" "$output_in_container" "${mapped_args[@]}"