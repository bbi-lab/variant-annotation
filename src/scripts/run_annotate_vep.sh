...
if [[ $# -lt 2 ]]; then
  cat <<'EOF'
Usage: src/scripts/run_annotate_vep.sh <input-file> <output-file> [annotate_vep options...]

Examples:
  src/scripts/run_annotate_vep.sh mapped.tsv annotated.tsv
  src/scripts/run_annotate_vep.sh mapped.tsv annotated.tsv --mapped-hgvs-g-col mapped_hgvs_g
...
Notes:
  ...
  - By default annotate_vep reads genomic HGVS values from mapped_hgvs_g.
  - Use --mapped-hgvs-g-col to use a different input column name.
  - Images are reused by default for fast runs.
  - Add --rebuild-image to force rebuilding the annotate-vep image.
...
exec docker compose --profile tools run \
  ...
  annotate-vep "$input_in_container" "$output_in_container" "$@"
