...
| `mavedb.primary_calibration.functional_class` | Functional classification label for this variant under the primary calibration |
...
| `mavedb.investigator_provided_calibration.functional_class` | Functional classification label under the investigator-provided calibration |

All eight columns are always written, defaulting to empty string when no applicable calibration exists or the variant is unclassified.
...
The calibration's `functionalClassifications` entries each have a `range` field. The variant's numeric score is evaluated against each range in order; the first matching range's `label` is returned.
...
The numeric score is taken from `--score-col`; a missing or non-numeric score results in an empty `functional_class`.

### Class-based

The calibration's `functionalClassifications` entries have no `range` field. The API endpoint `GET /api/v1/score-calibrations/{urn}/variants` is called to retrieve the full variant-to-class mapping for that calibration. The variant is looked up by its URN; if not present, `functional_class` is left empty.
...
**All `functional_class` columns are empty**
