#!/usr/bin/env bash
# Usage: examples/mavedb_and_igvf_data_prep/generate_mavedb_and_igvf_files.sh [data-dir]
#
# Generic copy of src/scripts/generate_mavedb_and_igvf_files.sh — same pipeline,
# reusing the same underlying src/*.py tools, just pointed at a different default
# data directory. Point it at any directory of Excel/CSV/TSV MAVE/SGE source files by
# passing that directory as $1; with no argument it processes the small example
# dataset that ships alongside this script (examples/mavedb_and_igvf_data_prep/data).
# See README.md in this directory for a walkthrough of what's in the example data
# and what each special case looks like before/after.
#
# Regenerates both the IGVF TSV and the MaveDB variant/scores/counts/functional-
# classes CSVs for every Excel/CSV/TSV source file in <data-dir>.
#
...
DATA_DIR="${1:-examples/mavedb_and_igvf_data_prep/data}"
