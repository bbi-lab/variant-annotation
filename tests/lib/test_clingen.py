...
import variant_annotation.lib.clingen as clingen
from variant_annotation.lib.clingen import resolve_clinvar_allele_id
...
# ---------------------------------------------------------------------------
# query_clingen_by_hgvs / resolve_clinvar_allele_id / resolve_clinvar_ids
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# resolve_clinvar_allele_id (direct, cache-first)
# ---------------------------------------------------------------------------
...
class TestResolveClinvarAlleleId:
    ...
    def test_404_returns_empty_string(self):
        ...
        with patch("variant_annotation.lib.clingen.requests.get", return_value=mock_resp):
    ...
    def test_client_error_no_retry(self):
        ...
        with patch("variant_annotation.lib.clingen.requests.get") as mock_get:
            mock_get.return_value = mock_resp
            mock_resp.raise_for_status.side_effect = http_error
            result = resolve_clinvar_allele_id("CA_BAD", cache, max_retries=3, retry_delay=0)

        assert result == ""


# ---------------------------------------------------------------------------
# _extract_grch38_coordinates
# ---------------------------------------------------------------------------
